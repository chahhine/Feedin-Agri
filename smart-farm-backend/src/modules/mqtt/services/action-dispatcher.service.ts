...
export class ActionDispatcherService {
  ...
  private async processAction(action: string, context: ActionContext): Promise<boolean> {
    ...
    switch (actionType) {
      ...
      case 'mqtt':
        // Classify action type based on violation type and action command
        const actionType = this.classifyAutomatedActionType(context, actionParam);
        return this.publishMqttMessage(actionParam, context, actionType);
    ...
    try {
      ...
      try {
        ...
        if (actionType === 'critical' || this.requiresConfirmation(actionCommand)) {
          this.setupConfirmationTimeout(saved.id, actionId, actionCommand, context.deviceId);
        } else {
   ...
   */
  private setupConfirmationTimeout(actionLogId: number, actionId: string, actionCommand: string, deviceId: string): void {
    const timeoutMs = 30000; // 30 seconds timeout
    
    setTimeout(async () => {
      try {
        const actionLog = await this.actionLogRepo.findOne({ where: { id: actionLogId } });
  ...
  async executeManual(actionUri: string, context: ActionContext, actionType: 'critical' | 'important' | 'normal' = 'normal', frontendActionId?: string): Promise<boolean> {
    // Expecting actionUri like 'mqtt:smartfarm/actuators/<device>/<action>'
    ...
    // Execute the action with production-ready settings
    const success = await this.publishMqttMessage(topic, manualContext, finalActionType, frontendActionId);
    
    if (success) {
      this.logger.log(`✅ Manual action executed successfully: ${actionUri} (QoS: ${this.getQosLevel(finalActionType)}, Retain: ${this.getRetainFlag(finalActionType)})`);
    } else {
      this.logger.error(`❌ Manual action execution failed: ${actionUri}`);
    }
