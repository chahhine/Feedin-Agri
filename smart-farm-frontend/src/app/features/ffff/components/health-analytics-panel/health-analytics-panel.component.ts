...
export class HealthAnalyticsPanelComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    // Use longer delay to prevent re-initialization loops
    setTimeout(() => {
  ...
  // Memoization cache for chart data - bounded per metric to prevent memory growth
  private soilCache = new Map<string, any>();
  private temperatureCache = new Map<string, any>();
  private humidityCache = new Map<string, any>();
  private sunlightCache = new Map<string, any>();
  private readonly MAX_CACHE_SIZE = 5; // Limit cache size to prevent memory growth

  // Helper to create stable cache key from data
  private createCacheKey(prefix: string, data: { timestamp: Date; value: number }[]): string {
    if (data.length === 0) return `${prefix}-empty`;
    // Use first and last timestamps + length for stable key
  ...
  soilMoistureChartData = computed(() => {
    ...
    if (!this.soilCache.has(cacheKey)) {
      ...
      this.soilCache.set(cacheKey, [{
        name: 'Soil Moisture',
        series: data.map(d => ({ 
          name: d.timestamp instanceof Date ? d.timestamp.toISOString() : String(d.timestamp), 
          value: d.value 
        }))
      }]);
    }
  ...
  temperatureChartData = computed(() => {
    ...
    const cacheKey = this.createCacheKey('temp', data);
    if (!this.temperatureCache.has(cacheKey)) {
      this.limitCacheSize(this.temperatureCache);
      this.temperatureCache.set(cacheKey, [{
        name: 'Temperature',
        series: data.map(d => ({ 
          name: d.timestamp instanceof Date ? d.timestamp.toISOString() : String(d.timestamp), 
          value: d.value 
  ...
  humidityChartData = computed(() => {
    ...
    const cacheKey = this.createCacheKey('humidity', data);
    if (!this.humidityCache.has(cacheKey)) {
      this.limitCacheSize(this.humidityCache);
      this.humidityCache.set(cacheKey, [{
        name: 'Humidity',
        series: data.map(d => ({ 
          name: d.timestamp instanceof Date ? d.timestamp.toISOString() : String(d.timestamp), 
          value: d.value 
  ...
  sunlightChartData = computed(() => {
    ...
    const cacheKey = this.createCacheKey('sunlight', data);
    if (!this.sunlightCache.has(cacheKey)) {
      this.limitCacheSize(this.sunlightCache);
      this.sunlightCache.set(cacheKey, [{
        name: 'Sunlight',
        series: data.map(d => ({ 
          name: d.timestamp instanceof Date ? d.timestamp.toISOString() : String(d.timestamp), 
          value: d.value 
    ...
    return this.sunlightCache.get(cacheKey)!;
  });
}
